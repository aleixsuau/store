  ...
  try {
    ...
    if (update && update.status) {
      if (update.status === 'active') {
        ...
        const currentContract: IContract = contractsCatalogue.find(contract => contract.Id === contractId);
        const currentClientContractResponse = await DDBB
                                                      .doc(`business/${siteId}/contracts/${contractId}/clients/${clientId}`)
                                                      .get();
        const currentClientContract = currentClientContractResponse.data() as IMindBroClientContract;
        console.log('updateContract currentClientContract', currentClientContract)

        if (currentClientContract.status === 'paused') {
          ...
          const today = appConfig.test && appConfig.today_test_mock ?
                          ...
                          moment();
          const lastAutopay = moment(currentClientContract.last_autopay);
          const nextAutopay = _getNextAutopayDate(currentContract, lastAutopay, appConfig);
          ...
          if (today.isSameOrAfter(nextAutopay)) {
            const skipPayment = false;
            const skipDeliver = !_isTodayTheAutopayDay(currentContract, null, null, appConfig);
            const contractOrder = await _processOneContractOrder(currentContract, clientId, currentClientContract, appConfig, token, null, skipPayment, skipDeliver);
            console.log('updateContract contractOrder', contractOrder)
            await _updateContractAfterProcessOrder(appConfig, contractOrder, currentClientContract, currentContract);
            await DDBB.doc(`business/${siteId}/clients/${clientId}`).update({[`contracts.${contractId}.status`]: update.status});
          } else {
            update = {
              ...update,
              start_date: _getFirstAutopayDate(currentContract, appConfig, true).toISOString(),
            }
            await DDBB.doc(`business/${siteId}/clients/${clientId}`).update({[`contracts.${contractId}.status`]: update.status});
            await DDBB.doc(`business/${siteId}/contracts/${contractId}/clients/${clientId}`).update(update);
          }
        } else if (currentClientContract.status === 'paused_no_payment') {
          const skipPayment = false;
...
async function _updateContractAfterProcessOrder(appConfig: IAppConfig, contractOrder: IOrder, clientContract: IMindBroClientContract, contract: IContract) {
  ...
  if (contractOrder.payment_status === 'approved') {
    ...
    /* if (clientContract.status === 'paused_no_payment') {
      const canceledOrder = _findOrderByStatus(appConfig, clientContract.id, clientContract.client_id, 'canceled');
      clientContractUpdate = {
        ...clientContractUpdate,
        status: contractOrder.delivered ? 'active' : 'activation_pending',
        last_autopay: null
 ...
 */
    ...
    if (!skipPayment) {
      clientContractUpdate = {
        ...
        // and the clientContractUpdate.status === 'activation_pending'
        ...
        // when the IContract is purchased, and then when arrives the firs IContract.clientsChargeOn
        last_autopay: clientContractUpdate && clientContractUpdate.status === 'activation_pending' ?
                          _getFirstAutopayDate(contract, appConfig).toISOString() :
                          // TEST FUNCTIONALITY
                          appConfig.test && appConfig.today_test_mock?
                            appConfig.today_test_mock :
...
/* async function _updateClientAutopaysStatus(appConfig: IAppConfig, contract: IContract, clientId: string, clientContract?: IMindBroClientContract) {
  ...
  if (updatedContractClient.autopays_counter === 0) {
    if (contract.ActionUponCompletionOfAutopays === 'ContractAutomaticallyRenews') {
      ...
      await DDBB
            .doc(`business/${appConfig.id}/clients/${clientContract.client_id}/contracts/${contract.Id}`)
